...
const COLLECTION_ADDRESSES = {
  ...
  dadbro: '0x06d0874b8f84e62a6943d6f800b8691ec0cd4fa9',
  bonkler: '0xabfae8a54e6817f57f9de7796044e9a60e61ad67'
};
...
export const filterNFTsByCollection = (nfts, collectionAddress) => {
  ...
  // Special debugging for DADBROS
  if (collectionAddress.toLowerCase() === '0x06d0874b8f84e62a6943d6f800b8691ec0cd4fa9') {
    console.log('=== DADBROS DEBUG ===');
    console.log('Looking for DADBROS contract:', collectionAddress);
    console.log('Total NFTs to filter:', nfts.length);
    console.log('DADBROS NFTs found:', filtered.length);

    // Show all unique contracts to see what we have
    const uniqueContracts = [...new Set(nfts.map(nft => nft.contract))];
    console.log('All unique contracts in wallet:', uniqueContracts);

    // Show DADBROS NFTs specifically
    if (filtered.length > 0) {
      console.log('DADBROS NFTs found:', filtered.map(nft => ({
        name: nft.name,
        contract: nft.contract,
        identifier: nft.identifier
      })));
    } else {
      console.log('No DADBROS NFTs found - checking for similar contracts...');
      const similarContracts = uniqueContracts.filter(contract =>
        contract && contract.toLowerCase().includes('06d0874b8f84e62a6943d6f800b8691ec0cd4fa9'.slice(0, 10))
      );
      console.log('Similar contracts found:', similarContracts);
    }
    console.log('=== END DADBROS DEBUG ===');
  }
  ...
  console.log(`Filtering NFTs for collection ${collectionAddress}:`, {
    ...
    sampleFilteredNFTs: filtered.slice(0, 2).map(nft => ({
      name: nft.name,
...
export const getAllCollectionNFTs = async (walletAddress) => {
  ...
  return {
    ...
    dadbros: filterNFTsByCollection(allNFTs, COLLECTION_ADDRESSES.dadbro),
    ...
    bonklers: filterNFTsByCollection(allNFTs, COLLECTION_ADDRESSES.bonkler)
  };
};
...
export const getNFTImageUrl = (nft) => {
  ...
  if (!imageUrl) {
    console.log('No image URL found for NFT:', {
      name: nft.name,
      contract: nft.contract,
      collection: nft.collection,
      availableFields: Object.keys(nft)
    });
  }
...
export const getNFTName = (nft) => {
  ...
  // Debug logging for names
  if (!name || name === 'NFT #') {
    console.log('No name found for NFT:', {
      contract: nft.contract,
      collection: nft.collection,
      identifier: nft.identifier,
      availableFields: Object.keys(nft)
