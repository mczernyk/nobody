...
const ProductDetails = ({ product, products }) => {
  ...
  const handleChangeColor = (event) => {
    if (collection === 'originals' || collection === 'originals-outers'|| collection === 'misc' || collection === 'egirl') {
      ...
      if (name === "love tee" || name === "passenger tee" || name === "trust tee" || name === 'self tee' || name === 'planet tee' || name === 'vacation tee') {
        if (event.target.value === 'black') {
          setIndex(0)
          setPreview(0)
        }
      ...
      if (name === "embroidered flower hoodie") {
        if (event.target.value === 'green') {
          setIndex(0)
          setPreview(0)
        }
      ...
      if (name === "embroidered arch crewneck") {
        if (event.target.value === 'black') {
          setIndex(0)
          setPreview(0)
        }
        ...
        <div className="product-detail-desc">
          ...
          {(custom && name !== 'milady rave custom tee') &&
            ...
            <button type="button" className="btn" onClick={connectWallet}>
              ...
              {collection === 'milady-station' &&
                <div>
                  {miladyStations.length ?
                    (
                    <div>
                      <div className='quantity'>
                        <p>{miladyStations.length} NFTs found.</p>
                        {customChoice && <p>{customChoice} selected.</p>}
                      </div>
                      <NFTContainer nfts={miladyStations} product={product}/>
                    </div>
                    ):(
                      <div>
                      <p>no NFTs detected from this collection, try connecting your wallet again please :)</p>
